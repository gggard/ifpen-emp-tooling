...
/**
 * Wizard page to choose the model to filter.
 * 
 * @author <a href="mailto:nathalie.lepine@obeo.fr">Nathalie Lepine</a>
 * 
 */
public class ModelToFilterPage extends WizardPage {
	 ...
	 * @param pageName
	 */
	protected ModelToFilterPage(IStructuredSelection selection) {
		super("Model to filter selection");
		setTitle("Select a model to filter.");
		setDescription("Select the model on which filters are applied.");
	...
	public void createControl(Composite parent) {
		...
		if (selectedModel != null) {
			createURI = URI.createPlatformResourceURI(selectedModel.getFullPath()
					.toString(), true);
		...
		btnFromWorkspace.setLayoutData(new GridData(SWT.RIGHT, SWT.FILL, true,
				false, 1, 1));
		...
		btnFromFileSystem.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER,
				true, false, 1, 1));
		...
		if (wizardContainer instanceof WizardDialog) {
			((IFPFilterModelWizard) getWizard()).addPageChangingListener();
		}
	}
	...
	protected void handleBrowseFileSystem() {
		...
		if (fileName != null) {
			createURI = URI.createFileURI(filterPath + File.separator
					+ fileName);
			text.setText(createURI.toString());
		}
	...
	protected void handleBrowseWorkspace() {
		...
		if (file != null) {
			createURI = URI.createPlatformResourceURI(file.getFullPath()
					.toString(), true);
			text.setText(createURI.toString());
		}
	...
	protected boolean isValidURI() {
		if (createURI == null) {
			setErrorMessage("Model to filter is empty.");
			return false;
		}
		...
		try {
			...
			if (resource == null) {
				setErrorMessage("Resource '" + createURI.toString()
						+ "' does not exist.");
				return false;
			}
			resource = null;
